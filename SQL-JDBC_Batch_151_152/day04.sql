...
--> Primary Key olan tablo parent tablo
--> Foreign Key olan tablo child tablo

--SORU 1: doktorlar tablosuna gorev_yeri 'Meddem' olan veri girişi yapınız

insert into doktorlar values('Meddem','Burcu',147852,'noroloji');
...
insert into doktorlar values('Okmeydani','Hatice Kar',135780,'ortopedi'); -- bu sebeple bu kod error verir

--SORU 3: doktorlar tablosuna gorev_yeri null olan veri girişi yapınız
-- foreign key null deger kabul eder

insert into doktorlar values(null,'Mehmet',246478,'noroloji');

--> child tablo null kabul eder
select * from doktorlar;

--SORU 4: .hastaneler tablosundaki hastane_adi 'Meddem' olan veriyi siliniz.

--> direkt parent daki veri silinemez. once child da silinmeli
...
--> bir tabloda birden fazla sutuna foreign key kisitlamasi yapilabilir
...
delete from parent;  -- bu durumda child tablosundaki veriler de silinir
...
--> as ile gecici isimlendirme verdik. sutunun ismini enyuksek_fiyat yapti.
...
select round(avg(fiyat),3) as ortalama_fiyat from arac;

-- round --> ondalikli kisimdan kurtulmak icin kullanilan bir komuttur. yuvalama yapar
...
insert into arac values(106,null,'porsche',145000,4000,'otomatik');

select count(marka) as arac_sayisi from arac;  -- null i veri olarak kabul etmedi
